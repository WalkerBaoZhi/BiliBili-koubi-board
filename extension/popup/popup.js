...
function render(range) {
    ...
    if (arr.length === 0) {
        listEl.innerHTML = `<div style="font-size:12px;color:#666;">当前榜单暂无数据。</div>`;
        return;
    }
...
fetch(DATA_URL)
    ...
    .catch((err) => {
        console.error("[Koubi] load data error", err);
        listEl.innerHTML =
            '<div style="font-size:12px;color:#666;">加载数据失败，请稍后重试。</div>';
    });
